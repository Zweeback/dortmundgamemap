...
## Emitted only after every collision cell in the initial batch has crossed a
## real physics-frame boundary.
...
func get_current_cell_id(player_position: Vector3) -> String:
	var easting := _world_origin.x + player_position.x
	var northing := _world_origin.z - player_position.z
	for meta in _cells_meta:
		var bbox: Array = meta["bbox"] as Array
		if (
			easting >= float(bbox[0])
			and easting < float(bbox[2])
			and northing >= float(bbox[1])
			and northing < float(bbox[3])
		):
