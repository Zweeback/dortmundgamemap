...
signal spawn_resolved(position_3d: Vector3)
...
func reset_to_spawn() -> void:
	...
	if bool(safe.get("ok", false)):
		_apply_resolved_spawn(safe["position"] as Vector3)
		return
	_apply_resolved_spawn(_resolve_surface_position(_spawn))
...
func teleport_to(world_xz: Vector2) -> void:
	...
	if bool(safe.get("ok", false)):
		_apply_resolved_spawn(safe["position"] as Vector3)
		return
	_apply_resolved_spawn(_resolve_surface_position(candidate))
...
func _attempt_spawn_resolve() -> void:
	...
	if bool(safe.get("ok", false)):
		...
		_apply_resolved_spawn(resolved)
		_pending_spawn_resolve = false
		_spawn_resolve_active = false
...
func _validate_historic_bad_spawn() -> void:
	if _spawn_building_bounds.is_empty():
		push_error("SPAWN_REGRESSION_GUARD_FAILED: no building bounds were cached")
		return
